...
public class ActionVault : IAction
{
    ...
    public void Do(Model m)
    {
        ...
        if (mh.vaultState == ModelHumanoid.vaultStates.canVault)
        {
            ...
            {
                checkFacingDirection = Dhit.transform.gameObject == mh.lastVault.gameObject;
                if (Dhit.transform.gameObject == mh.lastVault.gameObject)
                {
                    break;
                }
            }
            if (checkFacingDirection)
            {
                ...
                Collider obsCol = mh.lastVault.GetComponent<Collider>();
                ...
                {
                    if (hit.transform.gameObject == mh.lastVault.gameObject)
                    {
                ...
                mh.startVault(objectivePoint + m.transform.forward * objectivePointOffset);
            }
        }
    }
